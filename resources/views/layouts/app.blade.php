...
<html lang="id"
      ...
      x-init="
        $watch('darkMode', val => $store.app.toggleDarkMode());
...
<body class="bg-gradient-to-br from-gray-50 to-gray-100 dark:from-navy-950 dark:to-navy-900 font-sans antialiased min-h-screen transition-colors duration-300"
    ...
    <div x-show="$store.app.isLoading"
        ...
        <div class="text-center">
            <div class="relative">
                <div class="w-20 h-20 border-4 border-navy-200 dark:border-navy-700 border-t-navy-600 dark:border-t-gold-500 rounded-full animate-spin"></div>
            ...
            <div class="flex justify-between h-16">
                ...
                <div class="flex items-center">
                    ...
                    <button @click="sidebarOpen = !sidebarOpen; $store.app.sidebarOpen = sidebarOpen"
                        ...
                        <svg class="w-6 h-6" :class="{ 'hidden': sidebarOpen }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="flex items-center ml-2 lg:ml-0">
                        <div class="w-10 h-10 bg-gradient-to-br from-navy-600 to-navy-800 dark:from-navy-500 dark:to-navy-700 rounded-xl flex items-center justify-center shadow-lg">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <button @click="$store.app.toggleDarkMode()"
                        ...
                        <svg x-show="!darkMode" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linecap="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"></path>
                        </svg>
                        <svg x-show="darkMode" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linecap="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707"></path>
                        </svg>
                    </button>
                    ...
                    <div x-data="dropdown()" class="relative">
                        <button @click="toggle()"
                            ...
                            <span class="absolute top-1 right-1 w-2.5 h-2.5 bg-gold-500 rounded-full animate-pulse"></span>
                        </button>
                        ...
                        <div x-show="open"
                            ...
                            <template x-for="notification in $store.app.notifications" :key="notification.id">
                                <div class="px-4 py-3 hover:bg-gray-50 dark:hover:bg-navy-700 transition-colors">
                                    ...
                                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1" x-text="formatDate(notification.time)"></p>
                                </div>
                            </template>
                    ...
                    <div x-data="dropdown()" class="relative">
                        <button @click="toggle()"
                            ...
                            <div class="flex items-center justify-center w-8 h-8 bg-gradient-to-br from-navy-100 to-navy-200 dark:from-navy-700 dark:to-navy-600 rounded-full">
                                <span class="text-sm font-bold text-navy-700 dark:text-navy-200 group-hover:scale-110 transition-transform">
                        ...
                        <div x-show="open"
                            ...
                            <div class="px-4 py-3 border-b border-gray-100 dark:border-navy-700">
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ Auth::user()->email ?? Auth::user()->nip ?? '' }}</p>
                            </div>
                            ...
                            <a href="#" class="flex items-center px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-navy-700 transition-colors">
                                <svg class="w-4 h-4 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="flex pt-16 min-h-screen">
        <!-- Sidebar -->
        <aside class="fixed inset-y-0 left-0 transform lg:relative lg:translate-x-0 transition-all duration-300 ease-in-out w-72 lg:w-80 bg-white dark:bg-navy-800 border-r border-gray-200 dark:border-navy-700 shadow-soft z-30"
            ...
            <div class="p-6 border-b border-gray-200 dark:border-navy-700">
                <div class="flex items-center space-x-4">
                    <div class="flex-shrink-0">
                        <div class="w-12 h-12 bg-gradient-to-br from-navy-500 to-navy-700 dark:from-navy-400 dark:to-navy-600 rounded-2xl flex items-center justify-center shadow-lg">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400 truncate mt-0.5">
                            @hasrole('superadmin') Super Administrator @endhasrole
                            @hasrole('admin') Administrator @endhasrole
                            @hasrole('user') Pegawai @endhasrole
                        </p>
                    </div>
                </div>
            </div>
            ...
            <!-- Sidebar Footer -->
            <div class="absolute bottom-0 left-0 right-0 p-4 border-t border-gray-200 dark:border-navy-700 bg-white dark:bg-navy-800">
        ...
        <!-- Main Content -->
        <main class="flex-1 w-full min-w-0 px-4 sm:px-6 lg:px-8 py-6 lg:py-8 bg-gray-50 dark:bg-navy-900 transition-colors duration-300">
            <!-- Breadcrumb -->
                    ...
                    <div class="bg-green-50 dark:bg-green-900/20 border-l-4 border-green-500 p-4 rounded-r-lg">
                        <div class="flex items-center">
                            <svg class="w-5 h-5 text-green-500 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linecap="round" stroke-width="2" d="M9 12l2 2 4-5m6 3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                            </svg>
    ...
    <div x-show="sidebarOpen"
         @click="sidebarOpen = false; $store.app.sidebarOpen = false"
