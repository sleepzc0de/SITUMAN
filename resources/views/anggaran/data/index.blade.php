...
@section('content')
    <div class="space-y-5">
        ...
        @php
            ...
            $summaryData = Anggaran::whereNull('kode_subkomponen')
                ->whereNull('kode_akun')
                ->selectRaw(
                    'SUM(pagu_anggaran) as total_pagu, SUM(total_penyerapan) as total_realisasi, SUM(tagihan_outstanding) as total_outstanding, SUM(sisa) as total_sisa',
                )
                ->first();
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="card-flat p-4 border-l-4 border-l-navy-500">
                ...
                <p class="mt-1.5 text-xl font-bold text-gray-900 dark:text-white truncate"
                    title="{{ number_format($totalPagu) }}">
            ...
            <div class="card-flat p-4 border-l-4 border-l-green-500">
                ...
                <p class="mt-1.5 text-xl font-bold text-green-600 dark:text-green-400 truncate"
                    title="{{ number_format($totalRealisasi) }}">
            ...
            <div class="card-flat p-4 border-l-4 border-l-amber-500">
                ...
                <p class="mt-1.5 text-xl font-bold text-amber-600 dark:text-amber-400 truncate"
                    title="{{ number_format($totalOutstanding) }}">
            ...
            <form method="GET" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                <div class="input-group">
                    ...
                    <select name="level" class="input-field" data-auto-submit>
                        ...
                        <option value="ro" {{ request('level') == 'ro' ? 'selected' : '' }}>RO (Parent)
                        </option>
                        <option value="subkomponen" {{ request('level') == 'subkomponen' ? 'selected' : '' }}>Sub Komponen
                        </option>
                        <option value="akun" {{ request('level') == 'akun' ? 'selected' : '' }}>Akun (Detail)
                        </option>
                    </select>
                </div>
                <div class="input-group sm:col-span-2">
                    ...
                    <div class="flex gap-2">
                        <input type="text" name="search" value="{{ request('search') }}"
                            placeholder="Kode akun, sub komponen, atau uraian..." maxlength="200"
                        ...
                        @if (request()->hasAny(['ro', 'level', 'search']))
                            <a href="{{ route('anggaran.data.index') }}" class="btn btn-ghost btn-icon"
                                title="Reset filter">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="card p-0 overflow-hidden">
            ...
            <div class="px-5 py-3 border-b border-gray-100 dark:border-navy-700 flex items-center justify-between">
                <p class="text-sm text-gray-500 dark:text-gray-400">
                    Menampilkan
                    <span
                        class="font-semibold text-gray-700 dark:text-gray-200">{{ $anggarans->firstItem() ?? 0 }}–{{ $anggarans->lastItem() ?? 0 }}</span>
                    dari <span class="font-semibold text-gray-700 dark:text-gray-200">{{ $anggarans->total() }}</span>
                    data
                </p>
                <div class="flex items-center gap-3 text-xs text-gray-400 dark:text-gray-500">
                    ...
                    <span class="flex items-center gap-1.5">
                        <span class="w-2.5 h-2.5 rounded-sm bg-purple-200 dark:bg-purple-900/60 inline-block"></span> Sub
                        Komponen
                    </span>
                    <span class="flex items-center gap-1.5">
                        <span
                            class="w-2.5 h-2.5 rounded-sm bg-white dark:bg-navy-800 border border-gray-200 dark:border-navy-600 inline-block"></span>
                        Akun
                    </span>
                </div>
            </div>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="bg-gray-50 dark:bg-navy-800/80 border-b border-gray-200 dark:border-navy-700">
                            <th
                                class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider w-10">
                                #</th>
                            <th
                                class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider min-w-[140px]">
                                Kode / Referensi</th>
                            <th
                                class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                                Uraian Program / Kegiatan</th>
                            <th
                                class="px-4 py-3 text-center text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider w-28">
                                Level</th>
                            <th
                                class="px-4 py-3 text-right text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider min-w-[130px]">
                                Pagu</th>
                            <th
                                class="px-4 py-3 text-right text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider min-w-[130px]">
                                Realisasi</th>
                            ...
                            <th
                                ...
                                Serapan</th>
                            <th
                                class="px-4 py-3 text-center text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider w-24">
                                Aksi</th>
                        </tr>
                    </thead>
                        ...
                        @forelse($anggarans as $index => $anggaran)
                            @php
                                ...
                                if ($isRO) {
                                    ...
                                    $rowClass =
                                        'bg-blue-50/50 dark:bg-blue-900/10 hover:bg-blue-100/60 dark:hover:bg-blue-900/20';
                                    $indentPx = '';
                                    ...
                                    $rowClass =
                                        'bg-purple-50/40 dark:bg-purple-900/10 hover:bg-purple-100/50 dark:hover:bg-purple-900/20';
                                    $indentPx = 'pl-5';
                                } else {
                                    $level = 'Akun';
                                ...
                                $persen =
                                    $anggaran->pagu_anggaran > 0
                                        ? round(($anggaran->total_penyerapan / $anggaran->pagu_anggaran) * 100, 1)
                                        : 0;
                                $sisaWarning =
                                    $anggaran->pagu_anggaran > 0 && $anggaran->sisa < $anggaran->pagu_anggaran * 0.2;

                                $barColor =
                                    $persen >= 80 ? 'bg-green-500' : ($persen >= 50 ? 'bg-amber-500' : 'bg-red-400');
                                $pctColor =
                                    $persen >= 80
                            ...
                            <tr class="transition-colors {{ $rowClass }}">
                                ...
                                <td class="px-4 py-3 {{ $indentPx }}">
                                    <div class="space-y-0.5 font-mono text-xs">
                                        ...
                                        <div class="flex items-center gap-1">
                                            ...
                                            <span
                                                class="font-bold text-navy-700 dark:text-navy-300">{{ $anggaran->ro }}</span>
                                        </div>
                                        {{-- Sub Komponen --}}
                                        @if ($anggaran->kode_subkomponen)
                                            <div class="flex items-center gap-1">
                                                ...
                                                <span
                                                    class="font-semibold text-purple-700 dark:text-purple-400">{{ $anggaran->kode_subkomponen }}</span>
                                            </div>
                                        @endif
                                        {{-- Akun --}}
                                        @if ($anggaran->kode_akun)
                                            <div class="flex items-center gap-1">
                                                ...
                                                <span
                                                    class="font-bold text-green-700 dark:text-green-400">{{ $anggaran->kode_akun }}</span>
                                            </div>
                                        @endif
                                    </div>
                                </td>
                                ...
                                <td class="px-4 py-3 text-right tabular-nums">
                                    ...
                                    @if ($sisaWarning)
                                        <div class="text-xs text-red-400 dark:text-red-500 whitespace-nowrap mt-0.5">⚠
                                            hampir habis</div>
                                    @endif
                                </td>
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex flex-col items-center gap-1.5">
                                        <span
                                            class="text-sm font-bold tabular-nums {{ $pctColor }}">{{ $persen }}%</span>
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-center gap-1">
                                        <a href="{{ route('anggaran.data.show', $anggaran) }}" class="table-action-view"
                                            title="Detail">
                                        ...
                                        <a href="{{ route('anggaran.data.edit', $anggaran) }}" class="table-action-edit"
                                            title="Edit">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        ...
                                        <div class="flex gap-2 mt-2">
                                            ...
                                            @else
                                                <a href="{{ route('anggaran.data.import-form') }}"
                                                    class="btn btn-outline btn-sm">Import Excel</a>
                                                <a href="{{ route('anggaran.data.create') }}"
                                                    class="btn btn-primary btn-sm">Tambah Manual</a>
                                            @endif
                                        </div>
                                    </div>
                                </td>
                    ...
                    @if ($anggarans->count() > 0)
                        <tfoot>
                            <tr class="bg-navy-50 dark:bg-navy-800/80 border-t-2 border-navy-200 dark:border-navy-600">
                                <td colspan="4"
                                    class="px-4 py-3 text-xs font-bold text-gray-600 dark:text-gray-300 uppercase tracking-wider">
                                    Total Halaman Ini
                                </td>
                                <td
                                    class="px-4 py-3 text-right font-bold text-gray-900 dark:text-white text-sm tabular-nums whitespace-nowrap">
                                    {{ format_rupiah($anggarans->sum('pagu_anggaran')) }}
                                </td>
                                <td
                                    class="px-4 py-3 text-right font-bold text-green-600 dark:text-green-400 text-sm tabular-nums whitespace-nowrap">
                                    {{ format_rupiah($anggarans->sum('total_penyerapan')) }}
                                </td>
                                <td
                                    class="px-4 py-3 text-right font-bold text-sm tabular-nums whitespace-nowrap
