...
@section('content')
    @php
        ...
        $persen = $data->pagu_anggaran > 0 ? round(($data->total_penyerapan / $data->pagu_anggaran) * 100, 1) : 0;
    @endphp
    ...
    --}}
            ...
            <div class="space-y-5">
                ...
                <div class="card-flat border-l-4 {{ $levelBorderColor }} p-4">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <div class="flex items-center gap-3">
                            <div
                                class="w-9 h-9 rounded-xl {{ $isRO ? 'bg-blue-100 dark:bg-blue-900/30' : ($isSubkomponen ? 'bg-purple-100 dark:bg-purple-900/30' : 'bg-green-100 dark:bg-green-900/30') }}
                            ...
                            <div class="flex items-center gap-4 text-right">
                                <div>
                                    ...
                                    <p class="text-sm font-bold text-gray-900 dark:text-white">
                                        {{ format_rupiah($data->pagu_anggaran) }}</p>
                                </div>
                                @if ($data->total_penyerapan > 0)
                                    <div>
                                        <p class="text-xs text-gray-400 dark:text-gray-500">Realisasi</p>
                                        <p class="text-sm font-bold text-green-600 dark:text-green-400">
                                            {{ format_rupiah($data->total_penyerapan) }}</p>
                                    </div>
                                    <div class="hidden sm:block">
                                        <p class="text-xs text-gray-400 dark:text-gray-500 text-right">Serapan</p>
                                        <p
                                            class="text-sm font-bold {{ $persen >= 80 ? 'text-green-600 dark:text-green-400' : ($persen >= 50 ? 'text-amber-600 dark:text-amber-400' : 'text-red-500 dark:text-red-400') }}">
                                            {{ $persen }}%
                                        </p>
                                    </div>
                                @endif
                    ...
                    <div class="alert-warning rounded-xl p-4">
                        <div class="flex gap-3">
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="font-semibold text-yellow-800 dark:text-yellow-300 text-sm">Perhatian — Item Ini
                                    Sudah Memiliki Realisasi</p>
                                <div class="mt-1.5 flex flex-wrap gap-4 text-sm text-yellow-700 dark:text-yellow-400">
                                    @if ($data->total_penyerapan > 0)
                                        <span>Realisasi:
                                            <strong>{{ format_rupiah($data->total_penyerapan) }}</strong></span>
                                    @endif
                                    @if ($data->tagihan_outstanding > 0)
                                        <span>Outstanding:
                                            <strong>{{ format_rupiah($data->tagihan_outstanding) }}</strong></span>
                                    @endif
                                </div>
                                @if ($isAkun)
                                    <p class="text-xs text-yellow-600 dark:text-yellow-500 mt-1.5">
                                        Mengubah pagu akan mempengaruhi sisa anggaran. Pastikan perubahan sudah sesuai
                                        dengan DIPA/revisi terbaru.
                                    </p>
                                @endif
                            </div>
                        </div>
                ...
                <div class="card">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" id="kro" name="kro" value="{{ old('kro', $data->kro) }}"
                ...
                <div class="card">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div class="input-group sm:col-span-2">
                            ...
                            <textarea id="program_kegiatan" name="program_kegiatan" rows="3" maxlength="1000"
                                ...
                                placeholder="Tuliskan uraian lengkap... (maks. 1000 karakter)" required>{{ old('program_kegiatan', $data->program_kegiatan) }}</textarea>
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" id="pic" name="pic" value="{{ old('pic', $data->pic) }}"
                    ...
                    @if ($isAkun)
                        <div id="pagu_change_preview"
                            class="mt-4 p-3 rounded-xl bg-navy-50 dark:bg-navy-900/40 border border-navy-100 dark:border-navy-700"
                            style="display:none;">
                            ...
                            <div class="grid grid-cols-3 gap-3 text-xs">
                                <div>
                                    ...
                                    <p class="font-semibold text-gray-700 dark:text-gray-300 mt-0.5">
                                        {{ format_rupiah($data->pagu_anggaran) }}</p>
                                </div>
                                <div>
                                    <p class="text-gray-500 dark:text-gray-400">Pagu Baru</p>
                                    <p class="font-semibold text-navy-700 dark:text-navy-300 mt-0.5"
                                        id="preview_new_pagu">—</p>
                                </div>
                                <div>
