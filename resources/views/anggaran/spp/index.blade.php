...
@section('content')
    <div class="space-y-5" x-data="sppIndex()" x-init="init()">
        ...
        <div id="stats-wrapper">
                ...
                <div class="stat-card border-l-4 border-blue-400">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="stat-card-sub text-gray-400 text-xs" id="stat-bruto-full">
                                {{ format_rupiah($totalBruto) }}</p>
                        </div>
                        <div
                ...
                <div class="stat-card border-l-4 border-navy-400">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="stat-card-sub text-gray-400 text-xs" id="stat-netto-full">
                                {{ format_rupiah($totalNetto) }}</p>
                        </div>
                        <div
                            class="w-10 h-10 bg-navy-100 dark:bg-navy-700 rounded-xl flex items-center justify-center flex-shrink-0">
                            <svg class="w-5 h-5 text-navy-600 dark:text-navy-300" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="stat-card border-l-4 border-emerald-400">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="stat-card-value text-xl text-emerald-600 dark:text-emerald-400" id="stat-sp2d">
                                {{ format_rupiah_short($totalSP2D) }}</p>
                            <p class="stat-card-sub text-emerald-500 text-xs" id="stat-sp2d-pct">
                                @if ($totalNetto > 0)
                                    {{ formatPersen($totalSP2D, $totalNetto) }}
                                @endif
                            </p>
                        </div>
                        <div
                ...
                <div class="stat-card border-l-4 border-orange-400">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="stat-card-value text-xl text-orange-600 dark:text-orange-400" id="stat-belum">
                                {{ format_rupiah_short($totalBelumSP2D) }}</p>
                            <p class="stat-card-sub text-orange-500 text-xs" id="stat-belum-pct">
                                @if ($totalNetto > 0)
                                    {{ formatPersen($totalBelumSP2D, $totalNetto) }}
                                @endif
                            </p>
                        </div>
                        <div
            ...
            {{-- Progress Bar SP2D --}}
            <div class="card-flat mt-4" id="progress-wrap" style="{{ $totalNetto <= 0 ? 'display:none' : '' }}">
                ...
                <div class="input-group">
                    ...
                    <select name="status" class="input-field" @change="onFilterChange()">
                        ...
                        <option value="Tagihan Telah SP2D"
                            {{ request('status') == 'Tagihan Telah SP2D' ? 'selected' : '' }}>
                            Sudah SP2D
                        </option>
                        <option value="Tagihan Belum SP2D"
                            {{ request('status') == 'Tagihan Belum SP2D' ? 'selected' : '' }}>
                            Belum SP2D
                        </option>
                    </select>
                </div>
                ...
                <div class="input-group">
                    ...
                    <div class="flex gap-2">
                        <input type="text" name="search" value="{{ request('search') }}"
                            placeholder="No SPP, Uraian, PIC…" maxlength="200" class="input-field"
                            @input="onSearchInput()">
                        <button type="button" x-show="hasFilters" x-transition @click="resetFilter()"
                            class="btn btn-ghost btn-icon flex-shrink-0" title="Reset filter">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="card-flat overflow-hidden">
            ...
            <div x-show="hasFilters" x-transition
            ...
            <div x-show="loading" x-transition:enter="transition-opacity duration-150"
                ...
                x-transition:leave-end="opacity-0" class="p-5 space-y-3">
                @for ($i = 0; $i < 5; $i++)
