    ...
    <div class="space-y-6">
        ...
        <div class="alert alert-warning">
            ...
            <div>
                ...
                <p class="text-sm mt-0.5">Perubahan hanya bisa dilakukan selama status masih <strong>Pending</strong>.
                    Pastikan data yang Anda ubah sudah benar sebelum menyimpan.</p>
            </div>
        </div>

        <form action="{{ route('anggaran.usulan.update', $usulan) }}" method="POST" x-data="{
            ro: '{{ old('ro', $usulan->ro) }}',
                    ...
                    <div class="card">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <select name="ro" x-model="ro" @change="fetchSubkomponen()"
                                class="input-field @error('ro') input-error @enderror" autocomplete="off" required>
                                <option value="">— Pilih RO —</option>
                                @foreach ($roList as $ro)
                                    <option value="{{ $ro }}"
                                        {{ old('ro', $usulan->ro) == $ro ? 'selected' : '' }}>
                                        {{ $ro }} – {{ get_ro_name($ro) }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <div class="input-group">
                                ...
                                <div class="relative">
                                    <select name="sub_komponen" x-model="subKomponen"
                                        ...
                                        <option value="">
                                            <span
                                                x-text="loading ? 'Memuat...' : (ro ? '— Pilih Sub Komponen —' : '— Pilih RO dulu —')"></span>
                                        </option>
                            ...
                            <div class="input-group">
                                ...
                                <select name="bulan" class="input-field @error('bulan') input-error @enderror" required>
                                    ...
                                    @foreach ($bulanList as $bulan)
                                        <option value="{{ $bulan }}"
                                            {{ old('bulan', $usulan->bulan) == $bulan ? 'selected' : '' }}>
                                            {{ ucfirst($bulan) }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="input-group">
                                ...
                                <div class="relative">
                                    <span
                                        class="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-gray-400 font-medium pointer-events-none">Rp</span>
                                    <input type="text" id="nilai_display"
                                        ...
                                        x-init="$el.value = nilaiUsulan ? parseInt(nilaiUsulan).toLocaleString('id-ID') : ''" autocomplete="off">
                            ...
                            <div class="input-group md:col-span-2">
                                ...
                                <textarea name="keterangan" rows="3" class="input-field @error('keterangan') input-error @enderror"
                                ...
                                <div class="flex justify-between items-center">
                                    ...
                                    <span class="text-xs font-bold tabular-nums"
                                        :class="parseFloat(selectedInfo?.sisa) < parseFloat(selectedInfo?.pagu_anggaran) * 0.2 ?
                                            'text-red-600 dark:text-red-400' : 'text-navy-700 dark:text-navy-300'"
                                ...
                                <div class="progress-bar-wrap">
                                    <div class="progress-bar bg-navy-500"
                                        :style="selectedInfo && selectedInfo.pagu_anggaran > 0 ?
                                            'width:' + Math.min(parseFloat(selectedInfo.total_penyerapan) / parseFloat(
                                                selectedInfo.pagu_anggaran) * 100, 100) + '%' :
                                            'width:0%'">
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="card">
                        ...
                        <div class="space-y-2 text-xs">
                            <div class="flex justify-between">
                                ...
                                <span
                                    class="font-mono text-gray-700 dark:text-gray-300">{{ substr($usulan->id, 0, 8) }}...</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-500">Dibuat</span>
                                <span
                                    class="text-gray-700 dark:text-gray-300">{{ format_datetime($usulan->created_at) }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-500">Diperbarui</span>
                                <span
                                    class="text-gray-700 dark:text-gray-300">{{ format_datetime($usulan->updated_at) }}</span>
                            </div>
                        </div>
                    </div>
                </div>
...
@push('scripts')
    <script>
        // Cegah manipulasi option value via DevTools
        document.addEventListener('DOMContentLoaded', function() {
            // Freeze semua option yang sudah ada di select
            document.querySelectorAll('select[name="ro"], select[name="bulan"]').forEach(function(sel) {
                // Simpan nilai yang valid saat halaman load
                const validValues = Array.from(sel.options).map(o => o.value);

                sel.addEventListener('change', function() {
                    // Jika nilai tidak ada dalam daftar asli, reset
