...
@section('content')
    <div class="space-y-6">
        ...
        <form action="{{ route('anggaran.usulan.update', $usulan) }}" method="POST" id="form-edit-usulan"
            x-data="usulanEditForm()" @submit.prevent="submitForm()" autocomplete="off" novalidate>
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-5">
                    <div class="card">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="input-group">
                                ...
                                <select name="ro" id="select-ro" x-model="ro" @change="onRoChange()"
                            ...
                            <div class="input-group">
                                ...
                                <div class="relative">
                                    <select name="sub_komponen" id="select-subkomponen" x-model="subKomponen"
                                        ...
                                        {{-- Option awal untuk nilai yang sudah tersimpan --}}
                                        @if ($usulan->sub_komponen)
                                            <option value="{{ e($usulan->sub_komponen) }}" selected>
                                                {{ e($usulan->sub_komponen) }}
                                            </option>
                                        @endif
                                    </select>
                            ...
                            <div class="input-group">
                                ...
                                <select name="bulan" id="select-bulan"
                                    class="input-field @error('bulan') input-error @enderror" required>
                            ...
                            <div class="input-group">
                                ...
                                <div class="relative">
                                    ...
                                    {{-- Display field — hanya untuk tampilan --}}
                                    <input type="text" id="nilai_display"
                                        class="input-field pl-9 @error('nilai_usulan') input-error @enderror"
                                        :class="{
                                            '!border-red-400 !ring-red-100 dark:!ring-red-900/30': sisaAnggaranWarning ===
                                                \'danger\',
                                            '!border-yellow-400 !ring-yellow-100 dark:!ring-yellow-900/30': sisaAnggaranWarning ===
                                                \'warning\'
                                        }"
                                        placeholder="0" @input="onNilaiInput($event)" x-init="$el.value = nilaiUsulan > 0 ?
                                            parseInt(nilaiUsulan).toLocaleString('id-ID') : ''"
                                        inputmode="numeric" autocomplete="off">
                                    {{-- Hidden field yang dikirim ke server --}}
                                    <input type="hidden" name="nilai_usulan" id="nilai_usulan_hidden"
                                        :value="nilaiUsulan">
                                </div>
                                <p x-show="sisaAnggaranWarning === 'danger'" class="input-hint-error" style="display:none;">
                                    ⚠ Nilai melebihi sisa anggaran sub komponen
                                </p>
                                <p x-show="sisaAnggaranWarning === 'warning'" class="input-hint-warning"
                                    style="display:none;">
                    ...
                    <div class="flex justify-end gap-3">
                        <a href="{{ route('anggaran.usulan.show', $usulan) }}" class="btn btn-outline">
                            Batal
                        </a>
                            ...
                            <div
                                ...
                                <div class="flex justify-between items-center">
                                    ...
                                    <span class="text-xs font-bold tabular-nums"
                                        :class="parseFloat(selectedInfo?.sisa) < parseFloat(selectedInfo?.pagu_anggaran) * 0.2 ?
                        ...
                        <div class="empty-state py-8">
                            <div class="empty-state-icon">
                                <svg class="w-8 h-8 text-gray-400" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 11h.01M12 11h.01M15
                                             11h.01M12 7h.01M3 5a2 2 0 012-2h14a2 2 0 012 2v14a2 2 0
...
@push('scripts')
    <script>
        function usulanEditForm() {
            // ── Data dari server (Blade) — tidak bisa dimanipulasi ──────
            ...
            // ── Whitelist dari server — satu-satunya sumber kebenaran ───
            ...
            return {
                ...
                init() {
                    ...
                    // Load subkomponen jika RO sudah ada
                    if (this.ro && this.isValidRo(this.ro)) {
                        this.fetchSubkomponen(true);
                    }
                },
                ...
                // ── Event Handlers ─────────────────────────────────────
                onRoChange() {
                    // Jika nilai RO tidak ada di whitelist, reject
                    if (this.ro && !this.isValidRo(this.ro)) {
                        this.ro = '';
                        document.getElementById('select-ro').value = '';
                        this.updateSubkomponenSelect([]);
                        if (typeof showToast === 'function') {
                            showToast('Nilai RO tidak valid.', 'error');
                        }
                        return;
                    }
                    this.fetchSubkomponen(false);
                },

                onSubkompChange(val) {
                    this.subKomponen = val;
                    this.selectedInfo = this.subkomponens.find(
                        s => s.kode_subkomponen === val
                    ) || null;
                },

                onNilaiInput(event) {
                    // Hanya terima digit, tolak apapun selain angka
                ...
                fetchSubkomponen(preselect = false) {
                    ...
                    fetch(
                        ...
                        .then(data => {
                            ...
                            this.updateSubkomponenSelect(data);

                            if (preselect && this.subKomponen) {
                                this.selectedInfo = data.find(
                ...
                // ── Update Select DOM ──────────────────────────────────
                // Dilakukan via DOM agar option list bersih dari manipulasi
                updateSubkomponenSelect(data) {
                    const sel = document.getElementById('select-subkomponen');
                    if (!sel) return;

                    // Bersihkan semua option
                    sel.innerHTML = '';
                    ...
                    // Tambah option dari data server
                    data.forEach(item => {
                        const opt = document.createElement('option');
                        // textContent auto-escape — aman dari XSS
                        opt.value = item.kode_subkomponen;
                        opt.textContent = `${item.kode_subkomponen} – ${item.program_kegiatan}`;
                        sel.appendChild(opt);
                    });

                    // Restore nilai yang dipilih jika masih valid
                    if (this.subKomponen) {
                        const exists = data.some(d => d.kode_subkomponen === this.subKomponen);
                        if (exists) {
                ...
                submitForm() {
                    ...
                    // Validasi RO dari whitelist
                    if (!roEl.value || !this.isValidRo(roEl.value)) {
                        if (typeof showToast === 'function') {
                            showToast('Nilai RO tidak valid. Silakan pilih ulang.', 'error');
                        }
                        roEl.focus();
                        return;
                    }

                    // Validasi Bulan dari whitelist
                    if (!bulanEl.value || !this.isValidBulan(bulanEl.value)) {
                        if (typeof showToast === 'function') {
                            showToast('Nilai bulan tidak valid. Silakan pilih ulang.', 'error');
                        }
                        bulanEl.focus();
                        return;
                    }
                    ...
                    if (!this.subKomponen) {
                        if (typeof showToast === 'function') {
                            showToast('Sub komponen wajib dipilih.', 'error');
                        }
                        return;
                    }
                    ...
                    // Submit ke server — server adalah pertahanan utama
