...
@section('content')
    <div class="space-y-6">
        ...
        <form action="{{ route('anggaran.usulan.store') }}" method="POST" x-data="{
            ro: '{{ old('ro') }}',
            ...
            fetchSubkomponen() {
                if (!this.ro) { this.subkomponens = [];
                    this.selectedInfo = null; return; }
                this.loading = true;
                this.subKomponen = '';
                this.selectedInfo = null;
                fetch(`{{ route('anggaran.usulan.ajax.subkomponen') }}?ro=${this.ro}`)
                    .then(r => r.json())
                    .then(data => { this.subkomponens = data;
                        this.loading = false; })
                    ...
                    <div class="card">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="input-group">
                                ...
                                <select name="ro" x-model="ro" @change="fetchSubkomponen()"
                                    ...
                                    @foreach ($roList as $ro)
                                        <option value="{{ $ro }}">{{ $ro }} – {{ get_ro_name($ro) }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('ro')
                            ...
                            <div class="input-group md:col-span-2">
                                <label class="input-label">Keterangan
                                    <span class="text-xs text-gray-400 ml-1"
                                        x-text="`(${(document.querySelector('[name=keterangan]')?.value.length ?? 0)}/500)`"></span>
                                </label>
                                <textarea name="keterangan" rows="3" maxlength="500"
                                    class="input-field @error('keterangan') input-error @enderror"
                                    placeholder="Jelaskan keperluan penarikan dana ini (opsional)... (maks. 500 karakter)">{{ old('keterangan') }}</textarea>
                                @error('keterangan')
                                    <p class="input-hint-error">{{ $message }}</p>
                                @enderror
                            </div>
                ...
                <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <div class="progress-bar-wrap">
                                    <div class="progress-bar"
                                        :class="selectedInfo && parseFloat(selectedInfo.total_penyerapan) / parseFloat(
                                                selectedInfo.pagu_anggaran) >= 0.8 ?
                    ...
                    <div class="card" x-show="!selectedInfo" x-transition>
                        <div class="empty-state py-8">
                            ...
                            <p class="empty-state-desc text-xs">Pilih RO dan Sub Komponen untuk melihat informasi sisa
                                anggaran</p>
                        </div>
                    </div>
