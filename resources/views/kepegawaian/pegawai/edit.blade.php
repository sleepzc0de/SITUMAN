        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <div class="flex items-center gap-2 mt-1">
                    <div
                        class="w-6 h-6 bg-gradient-to-br from-navy-500 to-navy-700 rounded-full flex items-center justify-center flex-shrink-0">
                        <span class="text-white text-xs font-bold uppercase">{{ substr($pegawai->nama, 0, 1) }}</span>
                    </div>
                    <p class="page-subtitle !mt-0">{{ $pegawai->nama }} · <span class="font-mono">{{ $pegawai->nip }}</span>
                    </p>
                </div>
            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div class="flex gap-5 mt-1.5">
                                @foreach (['Laki-laki', 'Perempuan'] as $jk)
                                    <label class="flex items-center gap-2 cursor-pointer group">
                                        ...
                                        <span
                                            class="text-sm text-gray-700 dark:text-gray-300 group-hover:text-navy-600 dark:group-hover:text-gold-400 transition-colors">{{ $jk }}</span>
                                    </label>
                                @endforeach
                            </div>
                        </div>
                        <div>
                            ...
                            <input type="date" name="tanggal_lahir"
                                value="{{ old('tanggal_lahir', $p->tanggal_lahir?->format('Y-m-d')) }}"
                                class="input-field">
                        </div>
                        <div>
                            <label class="input-label">Usia <span class="text-gray-400 font-normal">(tahun)</span></label>
                            <input type="number" name="usia" value="{{ old('usia', $p->usia) }}" class="input-field"
                                min="17" max="70">
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <select name="jenis_jabatan" class="input-field">
                                ...
                                @foreach (['Struktural', 'Fungsional', 'Pelaksana'] as $jj)
                                    <option value="{{ $jj }}"
                                        {{ old('jenis_jabatan', $p->jenis_jabatan) == $jj ? 'selected' : '' }}>
                                        {{ $jj }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="eselon" class="input-field">
                                ...
                                @foreach (['Eselon I', 'Eselon II', 'Eselon III', 'Eselon IV', 'Non Eselon'] as $e)
                                    <option value="{{ $e }}"
                                        {{ old('eselon', $p->eselon) == $e ? 'selected' : '' }}>{{ $e }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="jenis_pegawai" class="input-field">
                                ...
                                @foreach (['PNS', 'PPPK', 'Honorer'] as $jp)
                                    <option value="{{ $jp }}"
                                        {{ old('jenis_pegawai', $p->jenis_pegawai) == $jp ? 'selected' : '' }}>
                                        {{ $jp }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="status" class="input-field">
                                ...
                                @foreach (['AKTIF', 'CLTN', 'PENSIUN', 'NON AKTIF'] as $s)
                                    <option value="{{ $s }}"
                                        {{ old('status', $p->status) == $s ? 'selected' : '' }}>{{ $s }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            <label class="input-label">Grading <span
                                    class="text-gray-400 font-normal">(1–16)</span></label>
                            <div class="relative">
                                <input type="number" name="grading" value="{{ old('grading', $p->grading) }}"
                                    class="input-field pr-12" min="1" max="16">
                                <span
                                    class="absolute right-4 top-1/2 -translate-y-1/2 text-xs text-gray-400 pointer-events-none">/
                                    16</span>
                            </div>
                        </div>
                        <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="date" name="tmt_cpns"
                                value="{{ old('tmt_cpns', $p->tmt_cpns?->format('Y-m-d')) }}" class="input-field">
                        </div>
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                <div class="relative">
                                    <input type="number" name="masa_kerja_tahun"
                                        value="{{ old('masa_kerja_tahun', $p->masa_kerja_tahun) }}"
                                        class="input-field pr-14" min="0">
                                    <span
                                        class="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-gray-400 pointer-events-none">tahun</span>
                                </div>
                                <div class="relative">
                                    <input type="number" name="masa_kerja_bulan"
                                        value="{{ old('masa_kerja_bulan', $p->masa_kerja_bulan) }}"
                                        class="input-field pr-14" min="0" max="11">
                                    <span
                                        class="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-gray-400 pointer-events-none">bulan</span>
                                </div>
                            </div>
                        </div>
                        <div>
                            ...
                            <input type="date" name="tanggal_pensiun"
                                value="{{ old('tanggal_pensiun', $p->tanggal_pensiun?->format('Y-m-d')) }}"
                                class="input-field">
                        </div>
                        <div>
                            <label class="input-label">Tahun Pensiun</label>
                            <input type="number" name="tahun_pensiun"
                                value="{{ old('tahun_pensiun', $p->tahun_pensiun) }}" class="input-field"
                                min="{{ date('Y') }}">
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <select name="pendidikan" class="input-field">
                                ...
                                @foreach (['SD', 'SMP', 'SMA/SMK', 'D1', 'D2', 'D3', 'D4', 'S1', 'S2', 'S3'] as $pd)
                                    <option value="{{ $pd }}"
                                        {{ old('pendidikan', $p->pendidikan) == $pd ? 'selected' : '' }}>
                                        {{ $pd }}</option>
                                @endforeach
                            </select>
                        </div>
