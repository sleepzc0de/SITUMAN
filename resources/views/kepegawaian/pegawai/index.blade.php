...
@section('content')
    <div class="space-y-6 animate-fade-in" x-data="pegawaiIndex()" x-init="init()">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            @php
                $statCards = [
                    [
                        'label' => 'Total Pegawai',
                        'value' => number_format($analytics['total']),
                        'sub' => $analytics['aktif'] . ' aktif',
                        'icon' =>
                            'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z',
                        'bg' => 'bg-navy-100 dark:bg-navy-700',
                        'ico' => 'text-navy-600 dark:text-navy-300',
                        'val' => 'text-navy-700 dark:text-white',
                    ],
                    [
                        'label' => 'Pegawai Aktif',
                        'value' => number_format($analytics['aktif']),
                        'sub' => number_format($analytics['tidak_aktif']) . ' non-aktif',
                    ...
                    [
                        ...
                        'value' => $analytics['avg_grading'],
                        'sub' => 'Dari grade 1–16',
                        'icon' => 'M13 7h8m0 0v8m0-8l-8 8-4-4-6 6',
                        'bg' => 'bg-gold-100 dark:bg-gold-900/30',
                        'ico' => 'text-gold-600 dark:text-gold-400',
                        'val' => 'text-gold-700 dark:text-gold-400',
                    ],
                    [
                        'label' => 'Rata-rata Usia',
                        'value' => $analytics['avg_usia'] . ' th',
                        'sub' => 'Masa kerja ' . $analytics['avg_masa_kerja'] . ' th',
                    ...
                    [
                        'label' => 'Pensiun ≤1 Th',
                        'value' => $analytics['akan_pensiun_1th'],
                        'sub' => 'Segera pensiun',
                        'icon' => 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z',
                        'bg' => 'bg-red-100 dark:bg-red-900/30',
                        'ico' => 'text-red-500 dark:text-red-400',
                        'val' => 'text-red-600 dark:text-red-400',
                    ],
                    [
                        'label' => 'Pensiun ≤2 Th',
                        'value' => $analytics['akan_pensiun_2th'],
                        'sub' => 'Perlu diperhatikan',
                        'icon' =>
                            'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z',
                        'bg' => 'bg-orange-100 dark:bg-orange-900/30',
                        'ico' => 'text-orange-500 dark:text-orange-400',
                        'val' => 'text-orange-600 dark:text-orange-400',
                    ],
                ];
            @endphp
            ...
            <div class="card">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="w-8 h-8 bg-gold-100 dark:bg-navy-700 rounded-lg flex items-center justify-center">
                        <svg class="w-4 h-4 text-gold-600 dark:text-gold-400" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    ════════════════════════════════════════════════════════ --}}
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            @foreach ([['id' => 'chartJK', 'label' => 'Jenis Kelamin'], ['id' => 'chartPendidikan', 'label' => 'Pendidikan'], ['id' => 'chartUsia', 'label' => 'Rentang Usia'], ['id' => 'chartMasaKerja', 'label' => 'Masa Kerja']] as $chart)
                <div class="card p-4">
                    <h3 class="text-xs font-bold text-gray-700 dark:text-gray-300 mb-3 uppercase tracking-wider">
                        {{ $chart['label'] }}</h3>
            ...
            <div class="card p-0 overflow-hidden">
                ...
                <div class="divide-y divide-gray-50 dark:divide-navy-700/60 max-h-52 overflow-y-auto scrollbar-thin">
                    ...
                    @forelse($pensiunList as $pp)
                        ...
                        <div
                            class="px-4 py-2.5 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-navy-700/40 transition-colors">
                            <div class="flex items-center gap-2.5 min-w-0">
                                <div
                                    class="w-7 h-7 bg-red-100 dark:bg-red-900/30 rounded-lg flex items-center justify-center flex-shrink-0">
                                    <span
                                        class="text-xs font-bold text-red-600 dark:text-red-400">{{ substr($pp->nama, 0, 1) }}</span>
                                </div>
                                <div class="min-w-0">
                                    <p class="text-xs font-semibold text-gray-900 dark:text-white truncate max-w-32">
                                        {{ $pp->nama }}</p>
                            ...
                            <div class="text-right flex-shrink-0 ml-2">
                                <p
                                    class="text-xs font-bold {{ $bulanLagi <= 12 ? 'text-red-600 dark:text-red-400' : 'text-orange-500 dark:text-orange-400' }}">
                                    {{ $bulanLagi }} bln
                                </p>
                                <p class="text-xs text-gray-400">
                                    {{ \Carbon\Carbon::parse($pp->tanggal_pensiun)->format('m/Y') }}</p>
                            </div>
                        </div>
                    @empty
    ...
    ════════════════════════════════════════════════════════ --}}
        <div class="card p-0 overflow-hidden">
            ...
            <div class="px-5 py-4 border-b border-gray-100 dark:border-navy-700 bg-gray-50/60 dark:bg-navy-800/60">
                <div class="flex flex-wrap gap-3 items-end">
                    ...
                    <div class="flex-1 min-w-52">
                        <label
                            class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1.5">Cari</label>
                        <div class="relative">
                            ...
                            <input type="text" x-model="filters.search" @input="debouncedFetch()"
                                placeholder="Nama, NIP, jabatan..." maxlength="100" class="input-field pl-9">
                        </div>
                    </div>
                    ...
                    <div class="min-w-36">
                        <label
                            class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1.5">Bagian</label>
                    ...
                    <div class="min-w-32">
                        <label
                            class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1.5">Status</label>
                    ...
                    <div class="min-w-28">
                        <label
                            class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1.5">Gender</label>
                        <select x-model="filters.jenis_kelamin" @change="fetchData()"
                    ...
                    <div class="min-w-28">
                        <label
                            class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1.5">Pendidikan</label>
                    ...
                    <div class="min-w-28">
                        <label
                            class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1.5">Eselon</label>
                    ...
                    <div class="min-w-20">
                        <label
                            class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1.5">Per
                            hal.</label>
                        <select x-model="filters.per_page" @change="fetchData()" class="input-field py-2 px-3 text-sm">
                            @foreach ([10, 20, 50, 100] as $pp)
                                <option value="{{ $pp }}" {{ $pp == 20 ? 'selected' : '' }}>
                                    {{ $pp }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="flex items-end">
                        <button type="button" x-show="hasActiveFilter" @click="resetFilters()"
            ...
            {{-- ── Result Info Bar ──────────────────────────────── --}}
            <div
                class="px-5 py-3 flex items-center justify-between border-b border-gray-100 dark:border-navy-700 bg-white dark:bg-navy-800 min-h-[44px]">
                <p class="text-sm text-gray-500 dark:text-gray-400">
                    <template x-if="!loading">
                        <span>
                            Menampilkan
                            <span class="font-semibold text-gray-900 dark:text-white"
                                x-text="info.from + '–' + info.to"></span>
                            dari <span class="font-semibold text-gray-900 dark:text-white" x-text="info.total"></span>
                            pegawai
                        </span>
                    </template>
                    <template x-if="loading">
            ...
            <div class="overflow-x-auto relative">
                {{-- Loading overlay --}}
                <div x-show="loading" x-transition:enter="transition-opacity duration-150"
