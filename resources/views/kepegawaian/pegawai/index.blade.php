...
<div class="space-y-6 animate-fade-in">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white dark:bg-navy-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-navy-700">
            <h3 class="text-xs font-bold text-gray-900 dark:text-white mb-4 uppercase tracking-wide">{{ $chart['label'] }}</h3>
            ...
            <div class="divide-y divide-gray-50 dark:divide-navy-700 max-h-52 overflow-y-auto">
                ...
                <div class="px-4 py-2.5 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-navy-700/50">
                    <div class="flex items-center gap-2.5 min-w-0">
                        ...
                        <div class="min-w-0">
                            <p class="text-xs font-semibold text-gray-900 dark:text-white truncate max-w-28">{{ $pp->nama }}</p>
                            <p class="text-xs text-gray-400 dark:text-gray-500">{{ $pp->bagian ?? '-' }}</p>
                        </div>
                    </div>
                    <div class="text-right flex-shrink-0 ml-2">
                        ...
                        <p class="text-xs text-gray-400 dark:text-gray-500">{{ \Carbon\Carbon::parse($pp->tanggal_pensiun)->format('m/Y') }}</p>
                    </div>
                </div>
                @empty
    ...
    <div class="bg-white dark:bg-navy-800 rounded-2xl shadow-sm border border-gray-100 dark:border-navy-700 overflow-hidden">
        ...
        {{-- Filter Bar --}}
        <div class="px-6 py-4 border-b border-gray-100 dark:border-navy-700 bg-gray-50/50 dark:bg-navy-800/80">
            <form method="GET" class="flex flex-wrap gap-3 items-end">
                ...
                <div class="flex-1 min-w-52">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}"
                            ...
                            class="w-full pl-9 pr-4 py-2 text-sm border-2 border-gray-200 dark:border-navy-600 rounded-xl focus:border-navy-400 focus:ring-4 focus:ring-navy-100 dark:focus:ring-navy-700/50 bg-white dark:bg-navy-700 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 transition-all">
                    </div>
                </div>
                ...
                @php
                $selects = [
                    ['name' => 'bagian',       'label' => 'Bagian',    'options' => $bagianList->map(fn($v)=>['v'=>$v,'l'=>$v]),                                                          'min' => 'min-w-36'],
                    ['name' => 'status',       'label' => 'Status',    'options' => collect(['AKTIF','CLTN','PENSIUN','NON AKTIF'])->map(fn($v)=>['v'=>$v,'l'=>$v]),                      'min' => 'min-w-32'],
                    ['name' => 'jenis_kelamin','label' => 'Gender',    'options' => collect(['Laki-laki','Perempuan'])->map(fn($v)=>['v'=>$v,'l'=>$v]),                                   'min' => 'min-w-28'],
                    ['name' => 'pendidikan',   'label' => 'Pendidikan','options' => $pendidikanList->map(fn($v)=>['v'=>$v,'l'=>$v]),                                                      'min' => 'min-w-28'],
                    ['name' => 'eselon',       'label' => 'Eselon',    'options' => $eselonList->map(fn($v)=>['v'=>$v,'l'=>$v]),                                                          'min' => 'min-w-28'],
                ];
                @endphp
                @foreach($selects as $sel)
                <div class="{{ $sel['min'] }}">
                    <label class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1.5">{{ $sel['label'] }}</label>
                    <select name="{{ $sel['name'] }}" class="w-full py-2 px-3 text-sm border-2 border-gray-200 dark:border-navy-600 rounded-xl bg-white dark:bg-navy-700 text-gray-900 dark:text-white focus:border-navy-400 focus:ring-4 focus:ring-navy-100 dark:focus:ring-navy-700/50 transition-all">
                        <option value="">Semua</option>
                        @foreach($sel['options'] as $opt)
                        <option value="{{ $opt['v'] }}" {{ request($sel['name']) == $opt['v'] ? 'selected' : '' }}>{{ $opt['l'] }}</option>
                        @endforeach
                    </select>
                </div>
                @endforeach
        ...
        <div class="px-6 py-3 bg-white dark:bg-navy-800 border-b border-gray-100 dark:border-navy-700 flex items-center justify-between">
            ...
            @if(request()->anyFilled(['search','bagian','status','jenis_kelamin','pendidikan','eselon']))
            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-navy-100 dark:bg-navy-700 text-navy-700 dark:text-navy-300">
                Filter aktif
            </span>
            @endif
        </div>
                    ...
                    <tr class="hover:bg-navy-50/30 dark:hover:bg-navy-700/30 transition-colors">
                        ...
                        <td class="px-5 py-4">
                            ...
                            $sc = [
                                ...
                                'CLTN'      => 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400',
                                'PENSIUN'   => 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400',
                                'NON AKTIF' => 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400',
                            ];
                        ...
                        <td class="px-5 py-4">
                            ...
                            <p class="text-xs {{ $isSegera ? 'text-red-400 dark:text-red-500' : 'text-orange-400 dark:text-orange-500' }}">{{ $bulanLagi }} bln</p>
                            @elseif($bulanLagi <= 0)
                            <p class="text-xs text-gray-400 dark:text-gray-500">Sudah pensiun</p>
                            @else
                            <p class="text-xs text-gray-400 dark:text-gray-500">{{ $pensiun->format('d/m/Y') }}</p>
                            @endif
                            @else
                            <span class="text-gray-300 dark:text-gray-600 text-sm">—</span>
                            @endif
                        ...
                        <td class="px-5 py-4">
                            <div class="flex items-center justify-center gap-1">
                                <a href="{{ route('kepegawaian.pegawai.show', $p) }}"
                                    class="w-8 h-8 flex items-center justify-center rounded-lg text-navy-500 dark:text-navy-400 hover:bg-navy-100 dark:hover:bg-navy-700 transition-colors"
                                    title="Detail">
                                ...
                                <a href="{{ route('kepegawaian.pegawai.edit', $p) }}"
                                    class="w-8 h-8 flex items-center justify-center rounded-lg text-gold-600 dark:text-gold-400 hover:bg-gold-50 dark:hover:bg-navy-700 transition-colors"
                                    title="Edit">
                                ...
                                <form method="POST" action="{{ route('kepegawaian.pegawai.destroy', $p) }}"
                                    ...
                                    @csrf @method('DELETE')
                                    <button type="submit"
                                        class="w-8 h-8 flex items-center justify-center rounded-lg text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
                                        title="Hapus">
        ...
        @if($pegawai->hasPages())
        <div class="px-6 py-4 border-t border-gray-100 dark:border-navy-700 bg-gray-50/50 dark:bg-navy-800/80">
            {{ $pegawai->links() }}
        </div>
        @endif
    </div>
