...
@section('page_header')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="page-subtitle mt-1">Analisis cerdas berdasarkan masa jabatan & data kepegawaian · Tahun <span
                    id="header-tahun" class="font-semibold text-gray-700 dark:text-gray-300">{{ $tahun }}</span></p>
        </div>
        <button id="btn-export" class="btn btn-secondary self-start flex-shrink-0">
...
@section('content')
    <div class="space-y-6 animate-fade-in">
        ...
        {{-- ── Filter Bar ── --}}
        <div
            class="relative overflow-hidden rounded-2xl bg-white dark:bg-navy-800
            ...
            <div class="px-5 py-4">
                    ...
                    <div class="w-full sm:w-auto">
                        ...
                        <select id="filter-tahun" class="input-field w-full sm:w-28">
                            @for ($y = date('Y') - 1; $y <= date('Y') + 2; $y++)
                                <option value="{{ $y }}" @selected($tahun == $y)>{{ $y }}
                                </option>
                            @endfor
                        </select>
                    </div>
                    ...
                    <div class="w-full sm:w-auto">
                        ...
                        <select id="filter-bagian" class="input-field w-full sm:w-44">
                            ...
                            @foreach ($bagianList as $b)
                                <option value="{{ $b }}" @selected(($bagian ?? '') == $b)>{{ $b }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div
                    ...
                    <div id="filter-loading" class="hidden items-center gap-2">
                        <div
                            class="w-3.5 h-3.5 border-2 border-navy-300 border-t-navy-600
        ...
        <div class="grid grid-cols-1 xl:grid-cols-4 gap-6">
            ...
            <div class="xl:col-span-3">
                <div
                    class="overflow-hidden rounded-2xl bg-white dark:bg-navy-800
                        border border-gray-100 dark:border-navy-700 shadow-sm">

                    {{-- Table header --}}
                    <div
                        class="px-5 py-3.5 border-b border-gray-100 dark:border-navy-700
                        ...
                        <table class="w-full text-sm">
                            <thead>
                                <tr class="border-b border-gray-100 dark:border-navy-700/80">
                                    <th
                                        class="pl-6 pr-3 py-3 text-left text-xs font-bold text-gray-400
                                           dark:text-gray-500 uppercase tracking-widest w-10">
                                        #</th>
                                    <th
                                        class="px-3 py-3 text-left text-xs font-bold text-gray-400
                                    ...
                                    <th
                                        class="px-3 py-3 text-left text-xs font-bold text-gray-400
                                           dark:text-gray-500 uppercase tracking-widest min-w-[140px]">
                                        Masa Jabatan</th>
                                    <th
                                        class="px-3 py-3 text-left text-xs font-bold text-gray-400
                                           dark:text-gray-500 uppercase tracking-widest">
                                    ...
                                    <th
                                        ...
                                        Pertimbangan</th>
                                    <th
                                        class="pl-3 pr-6 py-3 text-left text-xs font-bold text-gray-400
                                           dark:text-gray-500 uppercase tracking-widest">
                                        Aksi</th>
                                </tr>
                            </thead>
            ...
            <div class="xl:col-span-1 space-y-5">
                ...
                {{-- Sebaran Bagian --}}
                <div
                    class="rounded-2xl bg-white dark:bg-navy-800
                        border border-gray-100 dark:border-navy-700 shadow-sm overflow-hidden">
                    <div
                        class="px-4 py-3.5 border-b border-gray-100 dark:border-navy-700
                            bg-gray-50/80 dark:bg-navy-750">
                        <div class="flex items-center gap-2">
                    ...
                    <div class="p-4" id="stats-bagian-container">
                        @include('kepegawaian.mutasi._stats_bagian', [
                            'statsBagian' => $statsBagian,
                            'proyeksi' => $proyeksi,
                        ])
                    </div>
                </div>
                ...
                {{-- Kriteria --}}
                <div
                    class="rounded-2xl border border-navy-200 dark:border-navy-700
                    ...
                    <div class="p-4 space-y-3">
                        ...
                        <div
                            ...
                            <svg class="w-3.5 h-3.5 text-gold-500 mt-0.5 flex-shrink-0" fill="none"
    ...
    <script>
        (function() {
            ...
            let state = {
                ...
                prioritas: '{{ in_array($prioritas ?? '', ['', 'tinggi', 'sedang', 'rendah']) ? $prioritas ?? '' : '' }}',
            };
            ...
            function buildExportUrl() {
                ...
                return '{{ route('kepegawaian.mutasi') }}?' + params.toString();
            }
            ...
            function _doFetch() {
                ...
                /* Update label tahun */
                if (elYearLabel) elYearLabel.textContent = '· ' + state.tahun;
                if (elHeaderTahun) elHeaderTahun.textContent = state.tahun;
            ...
            if (elSearch) {
                elSearch.addEventListener('input', function() {
                    ...
                    if (elClearSearch) elClearSearch.classList.toggle('hidden', !state.search);
                    fetchData();
                });
            }
            ...
            /*
             * Export: gunakan window.location.href langsung.
             * Excel::download() mengembalikan BinaryFileResponse (file download),
             * BUKAN redirect — jadi window.location.href benar.
             * Tidak perlu event.preventDefault() atau fetch API untuk ini.
             */
            if (elBtnExport) {
                elBtnExport.addEventListener('click', function(e) {
                    e.preventDefault();
