        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            @php
                $statCards = [
                    ...
                    [
                        'label' => 'PIC & Lainnya',
                        'value' => $users->total() - ($roleCounts['superadmin'] ?? 0) - ($roleCounts['admin'] ?? 0),
                        'color' => 'green',
            ...
            <form method="GET" class="flex flex-col md:flex-row gap-3">
                <div class="relative flex-1">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="Cari nama, email, atau NIP..." class="input-field pl-9">
                </div>

                <select name="role" class="input-field md:w-52">
                    <option value="">Semua Role</option>
                    @foreach ($filterRoles as $value => $label)
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-9 h-9 rounded-full bg-gradient-to-br from-navy-500 to-navy-700
                                        ...
                                        <div class="min-w-0">
                                            ...
                                            @if (!$user->canBeDeleted())
                                                <span
                                                    class="text-xs font-medium text-red-500 dark:text-red-400 flex items-center gap-1">
                                ...
                                <td>
                                    <code
                                        class="text-xs bg-gray-100 dark:bg-navy-700 text-gray-600 dark:text-gray-300
                                ...
                                <td>
                                    <div class="flex items-center justify-center gap-1">
                                        <a href="{{ route('users.edit', $user) }}" class="table-action-edit"
                                            title="Edit user">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        ...
                                        @if ($user->canBeDeleted() && $user->id !== auth()->id())
                                            <button type="button" x-data="confirmDelete('{{ route('users.destroy', $user) }}', '{{ addslashes($user->nama) }}')" @click="submit()"
                                        ...
                                        @else
                                            <span
                                                class="w-7 h-7 flex items-center justify-center text-gray-300 dark:text-gray-600
                                             cursor-not-allowed"
                                                title="{{ $user->id === auth()->id() ? 'Tidak bisa hapus akun sendiri' : 'Akun terlindungi' }}">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
