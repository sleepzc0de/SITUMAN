...
@section('breadcrumb')
    <x-breadcrumb :items="[
        ...
        [
            'title' => $permintaanAtk->nomor_permintaan,
            'url' => route('inventaris.permintaan-atk.show', $permintaanAtk),
            'active' => false,
        ],
...
@section('page_header')
    <div>
        <h1 class="page-title">Edit Permintaan ATK</h1>
        <p class="page-subtitle">
            Nomor: <span
                class="font-mono font-semibold text-navy-600 dark:text-navy-300">{{ $permintaanAtk->nomor_permintaan }}</span>
            · <span class="badge-warning ml-1">Pending</span>
        </p>
    </div>
@endsection
            ...
            <div class="space-y-5">
                ...
                <div class="card">
                    <div class="section-header !mb-4">
                        <div>
                            ...
                            <p class="section-desc">Hanya permintaan berstatus <em>pending</em> yang dapat diedit</p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div class="input-group">
                            ...
                            <select name="pegawai_id" class="input-field @error('pegawai_id') input-error @enderror"
                                required>
                        ...
                        <div class="input-group">
                            ...
                            <input type="date" name="tanggal_permintaan"
                                ...
                                class="input-field @error('tanggal_permintaan') input-error @enderror" required>
                        ...
                        <div class="input-group md:col-span-2">
                            ...
                            <textarea name="keterangan" rows="2" maxlength="1000"
                                class="input-field @error('keterangan') input-error @enderror" placeholder="Keterangan tambahan (opsional)...">{{ old('keterangan', $permintaanAtk->keterangan) }}</textarea>
                            @error('keterangan')
                                <span class="input-hint-error">{{ $message }}</span>
                            @enderror
                        </div>
                ...
                <div class="card">
                    <div class="section-header !mb-4">
                        <div>
                            ...
                            <p class="section-desc">
                                ...
                                terdaftar — perubahan akan mengganti semua item sebelumnya
                            </p>
                        </div>
                    ...
                    <div class="hidden md:grid md:grid-cols-12 gap-3 px-1 mb-2">
                        <div
                            class="md:col-span-5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                            ATK</div>
                        <div
                            class="md:col-span-2 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                            Jumlah</div>
                        <div
                            class="md:col-span-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                            Keterangan</div>
                        ...
                        <template x-for="(item, index) in items" :key="index">
                            <div class="grid grid-cols-1 md:grid-cols-12 gap-3 p-3 rounded-xl bg-gray-50 dark:bg-navy-800/60 border border-gray-100 dark:border-navy-700"
                                ...
                                <div class="md:col-span-5">
                                    <label class="input-label text-xs md:hidden">ATK <span
                                    ...
                                    <select :name="'atk_id[' + index + ']'" class="input-field" required
                                        ...
                                        @foreach ($atk as $a)
                                            <option value="{{ $a->id }}">
                                                {{ $a->nama }}
                                                ({{ $a->kategori->nama ?? '-' }} · Stok: {{ $a->stok_tersedia }}
                                                {{ $a->satuan }})
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="md:col-span-2">
                                    <label class="input-label text-xs md:hidden">Jumlah <span
                                            class="text-red-500">*</span></label>
                                    <input type="number" :name="'jumlah[' + index + ']'" class="input-field" min="1"
                                        max="9999" placeholder="0" required x-model="item.jumlah">
                                </div>
                                <div class="md:col-span-4">
                                    <label class="input-label text-xs md:hidden">Keterangan</label>
                                    <input type="text" :name="'keterangan_item[' + index + ']'" class="input-field"
                                        maxlength="500" placeholder="Keterangan (opsional)" x-model="item.keterangan">
                                </div>
