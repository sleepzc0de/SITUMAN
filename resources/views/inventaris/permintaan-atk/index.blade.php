    ...
    <div class="space-y-5" x-data="permintaanIndex()" x-init="init()">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-5 gap-4">
            @php
                ...
                $colorMap = [
                    'navy' => [
                        'text' => 'text-navy-700 dark:text-white',
            ...
            @foreach ($statsConfig as $s)
                ...
                <div class="stat-card cursor-pointer transition-all duration-200"
                    :class="filters.status === '{{ $s['key'] === 'total' ? '' : $s['key'] }}' && '{{ $s['key'] }}'
                    !== 'total'
                        ?
                        ...
                        <div class="mt-3">
                            <div class="progress-bar-wrap">
                                <div class="progress-bar {{ $s['color'] === 'yellow' ? 'bg-yellow-500' : ($s['color'] === 'green' ? 'bg-green-500' : ($s['color'] === 'red' ? 'bg-red-500' : 'bg-blue-500')) }}"
                                    :style="'width: ' + (stats.total > 0 ? Math.round((stats.{{ $s['key'] }} / stats
                                        .total) * 100) : 0) + '%'">
                                </div>
                            </div>
        ...
        <div class="card-flat">
            <div class="flex flex-col lg:flex-row gap-3">
                ...
                <div class="relative flex-1">
                    ...
                    <input type="text" x-model="filters.search" @input.debounce.400ms="fetch()"
                        placeholder="Cari nomor permintaan atau nama pegawai..." maxlength="100" class="input-field pl-9">
                </div>
                ...
                <div class="flex gap-2">
                    <input type="date" x-model="filters.tanggal_dari" @change="fetch()"
                        class="input-field w-full lg:w-40" title="Dari tanggal">
                    <span class="self-center text-gray-400 text-sm flex-shrink-0">–</span>
                    <input type="date" x-model="filters.tanggal_sampai" @change="fetch()"
                        class="input-field w-full lg:w-40" title="Sampai tanggal">
                </div>

                {{-- Reset --}}
                <button type="button"
                    x-show="filters.search || filters.status || filters.tanggal_dari || filters.tanggal_sampai" x-transition
                    @click="resetFilters()" class="btn-ghost flex-shrink-0">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="card !p-0 overflow-hidden">
            ...
            {{-- Active filter chips --}}
            <div x-show="filters.search || filters.status || filters.tanggal_dari || filters.tanggal_sampai" x-transition
                            ...
                            <tr class="hover:bg-gray-50/70 dark:hover:bg-navy-700/30 transition-colors duration-150">
                                ...
                                <td>
                                    <div class="flex items-center gap-2.5">
                                        <div
                                            class="w-8 h-8 rounded-full bg-navy-100 dark:bg-navy-700 flex items-center justify-center flex-shrink-0">
                    ...
                    <template x-for="page in pageNumbers" :key="page">
                        <button @click="page !== '...' && goToPage(page)"
                            :class="page === meta.current_page ?
                                ...
                                'btn-ghost'"
                            class="btn-sm btn-icon min-w-[32px] rounded-lg text-xs font-semibold" x-text="page">
                        </button>
                    </template>
            ...
            return {
                ...
                async fetch() {
                    ...
                    try {
                        ...
                        const res = await axios.get('{{ route('inventaris.permintaan-atk.data') }}', {
                            params
                        });
