    ...
    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        ...
        <div class="flex flex-wrap items-center gap-2 flex-shrink-0">
            ...
            @if ($permintaanAtk->status == 'disetujui')
                @hasrole('superadmin|admin')
                    <form action="{{ route('inventaris.permintaan-atk.complete', $permintaanAtk) }}" method="POST"
            ...
            <div class="lg:col-span-2 space-y-5">
                ...
                <div class="card">
                    ...
                    <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
                        <div>
                            ...
                            <dd>
                                <div class="flex items-center gap-2.5">
                                    <div
                                        class="w-9 h-9 rounded-full bg-navy-100 dark:bg-navy-700 flex items-center justify-center flex-shrink-0">
                                    ...
                                    <div>
                                        ...
                                        @if ($permintaanAtk->pegawai?->jabatan)
                                            <p class="text-xs text-gray-500 dark:text-gray-400">
                                                {{ $permintaanAtk->pegawai->jabatan }}</p>
                                        @endif
                                    </div>
                                </div>
                            </dd>
                        ...
                        <div>
                            ...
                            <dd>
                                <p class="text-sm font-semibold text-gray-900 dark:text-white">
                                    {{ $permintaanAtk->user->nama ?? '-' }}</p>
                                <p class="text-xs text-gray-500 dark:text-gray-400">
                                    {{ format_tanggal($permintaanAtk->created_at, 'd F Y, H:i') }}</p>
                            </dd>
                        </div>
                        ...
                        @if ($permintaanAtk->penyetuju)
                            <div>
                                ...
                                <dd>
                                    <p class="text-sm font-semibold text-gray-900 dark:text-white">
                                        {{ $permintaanAtk->penyetuju->nama }}</p>
                                    <p class="text-xs text-gray-500 dark:text-gray-400">
                                        {{ format_tanggal($permintaanAtk->tanggal_disetujui, 'd F Y, H:i') }}</p>
                                </dd>
                            </div>
                        @endif
                        ...
                        @if ($permintaanAtk->keterangan)
                            <div class="sm:col-span-2">
                                <dt
                                    ...
                                    Keterangan</dt>
                                <dd
                                    class="text-sm text-gray-700 dark:text-gray-300 p-3 bg-gray-50 dark:bg-navy-800/60 rounded-lg">
                ...
                <div class="card !p-0 overflow-hidden">
                    <div class="px-5 py-4 border-b border-gray-100 dark:border-navy-700">
                        ...
                        <p class="section-desc">{{ $permintaanAtk->details->count() }} item ·
                            {{ $permintaanAtk->details->sum('jumlah') }} unit total</p>
                    </div>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <p class="text-xs text-gray-400 dark:text-gray-500 font-mono">
                                                {{ $detail->atk->kode_atk }}</p>
                                        </td>
                                        <td>
                                            <span
                                                class="badge-gray text-xs">{{ $detail->atk->kategori->nama ?? '-' }}</span>
                                        </td>
                                        <td class="text-center">
                                            <span
                                                class="text-sm font-bold text-gray-900 dark:text-white">{{ $detail->jumlah }}</span>
                                            <span class="text-xs text-gray-400 ml-1">{{ $detail->atk->satuan }}</span>
                                        </td>
                                        <td class="text-center">
                                            @php
                                                $stok = $detail->atk->stok_tersedia;
                                                $cukup = $stok >= $detail->jumlah;
                                            @endphp
                                            @if ($stok <= 0)
                                                <span class="badge-danger">Kosong</span>
                                            @elseif($cukup)
                                                <span class="badge-success">{{ $stok }}
                                                    {{ $detail->atk->satuan }}</span>
                                            @else
                                                <span class="badge-warning">{{ $stok }}
                                                    {{ $detail->atk->satuan }}</span>
                                            @endif
                                        </td>
                        ...
                        @foreach ($steps as $si => $step)
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                {{ $done ? 'bg-navy-600 dark:bg-navy-500' : 'bg-gray-100 dark:bg-navy-800 border-2 border-gray-200 dark:border-navy-700' }}">
                                    ...
                                    @if ($si < count($steps) - 1)
                                        <div
                                            class="w-0.5 h-5 {{ $currentOrder > $si ? 'bg-navy-500' : 'bg-gray-200 dark:bg-navy-700' }} my-0.5">
                                        </div>
                                    @endif
                                </div>
                                <div class="pb-1">
                                    <p
                                        class="text-sm font-semibold {{ $done ? 'text-gray-900 dark:text-white' : 'text-gray-400 dark:text-gray-500' }}">
                                        {{ $step['label'] }}
                            ...
                            <div class="flex items-start gap-2">
                                @php
                                    $stokKurang = $permintaanAtk->details
                                        ->filter(fn($d) => $d->atk->stok_tersedia < $d->jumlah)
                                        ->count();
                                @endphp
                                @if ($stokKurang > 0)
                                ...
                                @else
                                    <svg class="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" fill="none"
                                        stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
...
════════════════════════════════════════════════ --}}
    @hasrole('superadmin|admin')
        {{-- Approve Modal --}}
        <div id="approveModal"
            class="hidden fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" x-data
            @keydown.escape.window="document.getElementById('approveModal').classList.add('hidden')">
            <div
                class="bg-white dark:bg-navy-800 rounded-2xl shadow-2xl max-w-md w-full p-6 border border-gray-100 dark:border-navy-700">
            ...
            <div
                ...
                <form action="{{ route('inventaris.permintaan-atk.reject', $permintaanAtk) }}" method="POST">
                    ...
                    <div class="input-group mb-5">
                        ...
                        <textarea name="alasan_penolakan" rows="4" class="input-field" placeholder="Jelaskan alasan penolakan..."
                            required minlength="10" maxlength="500"></textarea>
                        <span class="input-hint">Alasan ini akan ditampilkan kepada pemohon. Min 10, max 500 karakter.</span>
                    </div>
