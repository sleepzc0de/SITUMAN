...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-5 gap-4">
        @php
        $statCards = [
            ['label' => 'Total Item',     'value' => $stats['total_item'],     'color' => 'navy',   'icon' => 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4'],
            ['label' => 'Stok Tersedia',  'value' => $stats['stok_tersedia'],  'color' => 'green',  'icon' => 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z'],
            ['label' => 'Stok Menipis',   'value' => $stats['stok_menipis'],   'color' => 'yellow', 'icon' => 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z'],
            ['label' => 'Stok Kosong',    'value' => $stats['stok_kosong'],    'color' => 'red',    'icon' => 'M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z'],
        ];
        $colorMap = [
            ...
            'yellow' => ['bg' => 'bg-yellow-100 dark:bg-yellow-900/30','icon' => 'text-yellow-600 dark:text-yellow-400','val' => 'text-yellow-600 dark:text-yellow-400'],
        ...
        <div class="card flex items-center gap-4 col-span-2 lg:col-span-1">
            ...
            <div class="min-w-0">
                ...
                <p class="text-lg font-bold text-gold-600 dark:text-gold-400 mt-0.5 truncate">{{ format_rupiah_short($stats['total_nilai']) }}</p>
            </div>
        </div>
    </div>
        ...
        <div class="flex flex-col xl:flex-row xl:items-end gap-4">
            ...
            <form method="GET" class="flex-1 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                <div class="input-group">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}"
                            placeholder="Nama atau kode ATK…"
                ...
                <div class="flex items-end gap-2">
                    ...
                    <a href="{{ route('inventaris.monitoring-atk.index') }}" class="btn-ghost btn-icon" title="Reset filter">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="flex items-center gap-2 flex-wrap xl:flex-nowrap xl:flex-shrink-0">
                ...
                <div x-data="{ open: false }" class="relative">
                    ...
                    <div x-show="open" x-cloak @click.away="open = false"
                        ...
                        <p class="px-4 pt-2 pb-1 text-[10px] font-bold text-gray-400 dark:text-navy-500 uppercase tracking-widest">Export</p>
                        <a href="{{ route('inventaris.monitoring-atk.export') }}" @click="open=false"
                           class="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-navy-700/60 transition-colors">
                            <span class="w-8 h-8 bg-green-100 dark:bg-green-900/30 rounded-lg flex items-center justify-center flex-shrink-0">
                        ...
                        <p class="px-4 pt-1 pb-1 text-[10px] font-bold text-gray-400 dark:text-navy-500 uppercase tracking-widest">Import</p>
                        ...
                        <a href="{{ route('inventaris.monitoring-atk.import-form') }}" @click="open=false"
                           class="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-navy-700/60 transition-colors rounded-b-xl">
                            <span class="w-8 h-8 bg-purple-100 dark:bg-purple-900/30 rounded-lg flex items-center justify-center flex-shrink-0">
    ...
    <div class="card !p-0 overflow-hidden">
        ...
        <div class="px-5 py-4 border-b border-gray-100 dark:border-navy-700 flex items-center justify-between gap-3">
            ...
            <div class="flex flex-wrap gap-1.5">
                ...
                @endif
                @if(request('kategori'))
                <span class="badge-info">Kat: {{ $kategoris->firstWhere('id', request('kategori'))?->nama }}</span>
                @endif
                @if(request('status'))
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <code class="text-xs bg-navy-50 dark:bg-navy-700 text-navy-700 dark:text-navy-300 px-2 py-0.5 rounded-md font-mono">
                                {{ $item->kode_atk }}
                            </code>
                        </td>
                        <td>
                            ...
                            @if($item->deskripsi)
                            <p class="text-xs text-gray-400 dark:text-gray-500 mt-0.5 line-clamp-2">{{ $item->deskripsi }}</p>
                            @endif
                        </td>
                        <td>
                        ...
                        <td>
                            @php
                                $pct = $item->stok_minimum > 0
                                    ...
                                    : 100;
                                $barColor = $item->status === 'tersedia' ? 'bg-green-500' : ($item->status === 'menipis' ? 'bg-yellow-500' : 'bg-red-500');
                            @endphp
                            <p class="font-semibold text-gray-900 dark:text-white text-sm">
                                {{ number_format($item->stok_tersedia) }}
                                <span class="text-xs font-normal text-gray-400">/ min {{ number_format($item->stok_minimum) }}</span>
                            </p>
                        ...
                        <td>
                            <div class="flex items-center justify-center gap-1">
                                ...
                                <button type="button"
                                    x-data="confirmDelete('{{ route('inventaris.monitoring-atk.destroy', $item) }}', '{{ addslashes($item->nama) }}')"
                                    @click="submit()"
