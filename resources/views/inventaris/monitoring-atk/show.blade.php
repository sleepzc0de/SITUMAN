        ...
        <div class="flex items-start gap-4">
            ...
            <div>
                ...
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <span class="text-xs text-gray-400">Diperbarui {{ $monitoringAtk->updated_at->diffForHumans() }}</span>
                </div>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="card">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    ...
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Harga Satuan</p>
                        <p class="font-medium text-gray-900 dark:text-white">{{ format_rupiah($monitoringAtk->harga_satuan) }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Dibuat</p>
                        <p class="font-medium text-gray-900 dark:text-white">{{ format_tanggal($monitoringAtk->created_at, 'd M Y') }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Diperbarui</p>
                        <p class="font-medium text-gray-900 dark:text-white">{{ format_tanggal($monitoringAtk->updated_at, 'd M Y H:i') }}</p>
                    </div>
                    @if($monitoringAtk->deskripsi)
                    <div class="col-span-2 sm:col-span-3">
                        <p class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Deskripsi</p>
                        <p class="text-gray-700 dark:text-gray-300 text-sm leading-relaxed">{{ $monitoringAtk->deskripsi }}</p>
                    </div>
                    @endif
                </div>
            </div>
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <code class="text-xs bg-gray-100 dark:bg-navy-700 text-gray-600 dark:text-gray-300 px-2 py-0.5 rounded font-mono">
                                ...
                                <td class="text-right font-semibold text-gray-900 dark:text-white tabular-nums">
                                    ...
                                    <span class="font-normal text-gray-400 text-xs">{{ $monitoringAtk->satuan }}</span>
                                </td>
                                <td>
                                    @php $st = $detail->permintaan->status @endphp
                                    @if($st === 'pending')     <span class="badge-warning">Pending</span>
                                    @elseif($st === 'disetujui') <span class="badge-success">Disetujui</span>
                                    @elseif($st === 'ditolak')  <span class="badge-danger">Ditolak</span>
                                    @else                        <span class="badge-info">Selesai</span>
                                    @endif
                                </td>
                            </tr>
                            @empty
        ...
        <div class="space-y-6">
            ...
            <div class="card">
                ...
                @php
                    $pct = $monitoringAtk->stok_minimum > 0
                        ? min(100, round($monitoringAtk->stok_tersedia / $monitoringAtk->stok_minimum * 100))
                        : 100;
                    $barColor = $monitoringAtk->status === 'tersedia' ? 'bg-green-500' : ($monitoringAtk->status === 'menipis' ? 'bg-yellow-500' : 'bg-red-500');
                @endphp
                ...
                <div class="space-y-4">
                    <div class="p-4 bg-navy-50 dark:bg-navy-700/50 rounded-xl text-center">
                        ...
                        <p class="text-4xl font-bold text-navy-700 dark:text-white">{{ number_format($monitoringAtk->stok_tersedia) }}</p>
                    ...
                    <div>
                        ...
                        <div class="progress-bar-wrap">
                            <div class="{{ $barColor }} h-full rounded-full transition-all duration-500" style="width: {{ min($pct, 100) }}%"></div>
                        </div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div class="p-3 bg-gold-50 dark:bg-gold-900/20 rounded-xl text-center">
                            ...
                            <p class="text-xl font-bold text-gold-700 dark:text-gold-400 mt-0.5">{{ $monitoringAtk->stok_minimum }}</p>
            ...
            <div class="card space-y-2">
                ...
                <a href="{{ route('inventaris.monitoring-atk.edit', $monitoringAtk) }}"
                   class="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-navy-700/60 transition-colors group">
                    <span class="w-8 h-8 bg-blue-100 dark:bg-blue-900/30 rounded-lg flex items-center justify-center flex-shrink-0">
                ...
                <a href="{{ route('inventaris.permintaan-atk.create') }}"
                   class="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-navy-700/60 transition-colors group">
                    <span class="w-8 h-8 bg-navy-100 dark:bg-navy-700 rounded-lg flex items-center justify-center flex-shrink-0">
                ...
                <button type="button"
                    x-data="confirmDelete('{{ route('inventaris.monitoring-atk.destroy', $monitoringAtk) }}', '{{ addslashes($monitoringAtk->nama) }}')"
                    @click="submit()"
                    class="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors text-left">
                    <span class="w-8 h-8 bg-red-100 dark:bg-red-900/30 rounded-lg flex items-center justify-center flex-shrink-0">
        ...
        <form action="{{ route('inventaris.monitoring-atk.update-stok', $monitoringAtk) }}" method="POST">
            ...
            <div class="modal-body space-y-4">
                ...
                <div class="input-group">
                    ...
                    <div class="grid grid-cols-2 gap-2" x-data="{ jenis: 'tambah' }">
                        <label class="cursor-pointer">
                            <input type="radio" name="jenis" value="tambah" class="sr-only" x-model="jenis" checked>
                ...
                <div class="input-group">
                    ...
                    <input id="jumlahStok" type="number" name="jumlah" class="input-field" min="1" required
                        placeholder="Masukkan jumlah…">
                </div>

                <div class="input-group">
                    <label class="input-label">Keterangan</label>
                    <textarea name="keterangan" rows="2" class="input-field"
